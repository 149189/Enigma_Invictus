...
const generateRefreshToken = (user) => {
    return jwt.sign({ user_id: user._id, role: user.role }, process.env.REFRESH_TOKEN_SECRET, {
        expiresIn: "7d"
    });
};
const cookieOptions = {
...
const registerUser = asyncHandler(async (req, res) => {
    ...
    let user = await Donor.findOne({ email, provider: "local" });
    if (user) {
        return sendResponse(
            res,
            false,
...
const resendVerificationOTP = asyncHandler(async (req, res) => {
    ...
    const user = await Donor.findOne({ email, provider: "local" });
    if (!user) {
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    const user = await Donor.findOne({ email, provider: "local" }).select("+password");
    if (!user) {
...
const sendVerificationOTP = asyncHandler(async (req, res) => {
    ...
    // Check if user already exists
    const existingUser = await Donor.findOne({ email, provider: "local" });
    if (existingUser) {
        return sendResponse(
            res,
            false,
